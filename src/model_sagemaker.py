...
# Map course indices to their text index sequences
course_text_indices = {}
...
for i in range(1, itemnum + 1):
    course_id = idx_to_course_id.get(i, None)
    if course_id:
        grade = course_grades.get(course_id, 0)
        grade_idx = grade_to_idx.get(grade, 0)
        course_idx_to_grade_idx[i] = grade_idx
    else:
        course_idx_to_grade_idx[i] = 0  # Unknown course index, set grade to 0

# Convert to TensorFlow constant
course_idx_to_grade_idx_tf = tf.constant(course_idx_to_grade_idx, dtype=tf.int32)

# 3. Process course data and student data
# Process course data
course_data = {
    'course_ids': list(course_id_to_idx.values()),  # Use index list
    'keywords': {}
}
...
for _, row in course_info.iterrows():
    ...
    if cid in course_id_to_idx:
        ...
        course_data['keywords'][idx] = row['Key Words']
    else:
        print(f"Course ID {cid} not found in course_id_to_idx.")
...
def process_student_data(merged_data, course_id_to_idx):
    """
    Process data into idx form

    Args:
        merged_data (DataFrame): union data for student info
        course_id_to_idx (dict): mapping ID and index

    Returns:
        list: dictionary for ID and index
    """
    student_data = []
...
student_data = process_student_data(merged_data, course_id_to_idx)

# 4. Define SASRec model (include course grade information and content embeddings)
...
if __name__ == "__main__":
    ...
    trainer = SASRecTrainer(
        usernum=usernum, itemnum=itemnum, gradenum=gradenum, vocab_size=vocab_size, args=args, mode="train"
    )
    ...
    s3_weights_key = "sasrec_weights.weights.h5"
    s3_mapping_key = "mapping_data.pkl" 

    trainer.model.save_weights("sasrec_weights.weights.h5")
    ...
    s3.upload_file("sasrec_weights.weights.h5", "cs583-source-data", s3_weights_key)    
    s3.upload_file("mapping_data.pkl", "cs583-source-data", s3_mapping_key)

# # 8. Evaluation
# def evaluate_model(model, test_inputs_seq, test_inputs_grades, test_inputs_contents, test_pos, course_data, k=10):
#     """
#     Evaluate the model on the test set using HR@k and NDCG@k.
    
#     Args:
#         model: The trained SASRec model.
#         test_inputs_seq: Test set sequences (shape: [num_samples, maxlen]).
#         test_inputs_grades: Test set grades (shape: [num_samples, maxlen]).
#         test_inputs_contents: Test set content embeddings (shape: [num_samples, maxlen, max_text_len]).
#         test_pos: Positive samples for test set (shape: [num_samples]).
#         course_data: Dictionary containing course IDs and additional data.
#         k: The number of top recommendations to evaluate (default: 10).
...
#     hr_sum = 0
#     ndcg_sum = 0
#     total_samples = len(test_inputs_seq)

#     for i in range(total_samples):
#         # Prepare test input
#         seq = tf.convert_to_tensor(test_inputs_seq[i].reshape(1, -1), dtype=tf.int32)  # Shape: [1, maxlen]
#         grades = tf.convert_to_tensor(test_inputs_grades[i].reshape(1, -1), dtype=tf.int32)  # Shape: [1, maxlen]
#         contents = tf.convert_to_tensor(test_inputs_contents[i].reshape(1, args.maxlen, max_text_len), dtype=tf.int32)

#         # Positive sample
#         target_item = test_pos[i]

#         # Candidate courses (all courses for evaluation)
#         candidate_items = tf.convert_to_tensor(course_data['course_ids'], dtype=tf.int32)
#         candidate_grades = tf.convert_to_tensor(course_idx_to_grade_idx[candidate_items], dtype=tf.int32)

#         # Use the model to compute sequence embedding
#         inputs = {'seq': seq, 'grades': grades, 'contents': contents}
#         seq_emb = model(inputs, training=False)[:, -1, :]  # Use the last time step embedding

#         # Compute embeddings for candidates
#         candidate_emb = model.item_emb(candidate_items)  # Shape: [num_candidates, hidden_units]
#         candidate_grade_emb = model.grade_emb(candidate_grades)  # Shape: [num_candidates, hidden_units]

#         # Combine embeddings (item + grade)
#         candidate_combined_emb = tf.concat([candidate_emb, candidate_grade_emb], axis=-1)  # Shape: [num_candidates, hidden_units * 2]

#         # Compute scores for candidates
#         scores = tf.matmul(seq_emb, candidate_combined_emb, transpose_b=True).numpy().flatten()  # Shape: [num_candidates]

#         # Sort candidates by score in descending order
#         top_k_items = np.argsort(-scores)[:k]  # Get top-k indices

#         # Calculate HR@k and NDCG@k
#         if target_item in top_k_items:
#             hr_sum += 1  # Hit Rate increment
#             rank = np.where(top_k_items == target_item)[0][0] + 1  # Rank of the target item (1-based)
#             ndcg_sum += 1 / np.log2(rank + 1)  # Discounted cumulative gain

#     # Calculate average metrics
#     hr = hr_sum / total_samples
#     ndcg = ndcg_sum / total_samples
...
#     test_inputs_seq, 
#     test_inputs_grades, 
#     test_inputs_contents, 
#     test_pos, 
#     course_data, 
#     k=10
# )

# print(f"Hit Rate (HR@10): {hr:.4f}")
